...
| :---: | :--- | :---: | :--- |
| **คนที่ 1** | ................................................................ | ................................... | ออกแบบฐานข้อมูล (ERD, 3NF), พัฒนาระบบคำสั่งซื้อและตะกร้าสินค้า, เขียน SQL รายงานที่ 1 และ 2 |
| **คนที่ 2** | ................................................................ | ................................... | พัฒนาระบบหลังบ้าน (Admin), ระบบความปลอดภัย Route Guard, เขียน SQL รายงานที่ 3 และ 4, ทดสอบระบบ (QA) |
...
ฐานข้อมูลของโปรเจกต์ `ebook-store` ประกอบด้วยตารางเชิงสัมพันธ์จำนวน **9 ตาราง** สอดคล้องตามข้อกำหนดของใบงาน (กำหนดขั้นต่ำ 8 ตาราง) โดยแสดงความสัมพันธ์ผ่านแผนผัง Mermaid ER Diagram ดังนี้:
...
1. **`roles` (1) <---> (N) `users`**: บทบาทหนึ่งบทบาท (เช่น customer, admin) สามารถกำหนดให้แก่ผู้ใช้งานได้หลายคน โดยเชื่อมโยงผ่าน `roles.role_id = users.role_id`
2. **`users` (1) <---> (1) `carts`**: สมาชิกแต่ละคนมีตะกร้าสินค้าประจำตัวคนละ 1 ตะกร้า โดยระบบจะสร้างแถวใน `carts` ให้อัตโนมัติเมื่อทำการสมัครสมาชิก
3. **`carts` (1) <---> (N) `cart_items`**: ตะกร้าสินค้า 1 ใบ สามารถบรรจุรายการหนังสือที่เตรียมสั่งซื้อได้หลายเล่ม
4. **`ebooks` (1) <---> (N) `cart_items`**: หนังสือ 1 เล่ม สามารถปรากฏอยู่ในตะกร้าสินค้าของผู้ใช้หลายคนได้
5. **`authors` (1) <---> (N) `ebooks`**: นักเขียน 1 ท่าน สามารถมีผลงานหนังสือในระบบได้หลายเล่ม โดยเชื่อมด้วย `authors.author_id = ebooks.author_id`
6. **`categories` (1) <---> (N) `ebooks`**: หมวดหมู่หนังสือ 1 หมวด สามารถจัดเก็บหนังสือได้หลายเล่ม
7. **`users` (1) <---> (N) `orders`**: สมาชิก 1 คน สามารถสร้างคำสั่งซื้อได้หลายครั้งในระบบ
8. **`orders` (1) <---> (N) `order_items`**: คำสั่งซื้อ 1 คำสั่งซื้อ ประกอบด้วยรายการหนังสือย่อยที่สั่งซื้ออย่างน้อย 1 รายการขึ้นไป
9. **`ebooks` (1) <---> (N) `order_items`**: หนังสือแต่ละเล่มสามารถถูกสั่งซื้อซ้ำในรายการย่อยของคำสั่งซื้อต่าง ๆ ได้หลายครั้ง
